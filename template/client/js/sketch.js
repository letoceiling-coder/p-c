...
////console.log(png_img)
    ...
    function ajax() {
        var room_area = elem_input_n4.value - 0;
        var perimeter = elem_input_n5.value - 0;
        var angles_count = elem_input_n9.value - 0;
        var goods = [{id: final_canvas_id, count: sq_polotna}]; //Кусок полотна
        var jobs = [
            {id: 60, count: room_area}, //60 Стоимость работы по кв. м.
            {id: 65, count: perimeter_shrink} //65 Гарпун
        ];
        var seamLength = 0;//макс.длина шва
        if (seam_lines.length) {
            seamLength = seam_lines[0].length;
            for (var i = seam_lines.length; i--;) {
                if (seam_lines[i].length > seamLength) {
                    seamLength = seam_lines[i].length;
                }
            }
        }

        seamLength = seamLength * p_usadki_final;//длина шва нужна усаженная

        if (seamLength < 7) {
            seamLength = 0;
        }
        if (texture == 25) {
            room_area = sq_polotna;
        }
        if (texture != 25) {
            if (angles_count > 4) {
                jobs.push({id: 59, count: angles_count - 4}); //59 Обработка доп. углов (>4) (себестоимость)
                if (angles_count > 6) {
                    jobs.push({id: 55, count: angles_count - 6}); //55 Обработка доп. углов (>6)
                }
            }
        }
        if (curvilinear_length > 0) {
            jobs.push({id: 61, count: curvilinear_length}); //61 Криволинейный участок (себестоимость)
            jobs.push({id: 62, count: curvilinear_length}); //62 Криволинейный участок
        }
        if (inner_cutout_length > 0) {
            jobs.push({id: 63, count: inner_cutout_length}); //63 Внутренний вырез в цеху
            jobs.push({id: 64, count: inner_cutout_length}); //64 Внутренний вырез в цеху (себестоимость)
        }

        if (!calc_id) {
            calc_id = selectedCalculation.id;
        }
        temp_ = texturesmanafacture.find(item => item.manufacturer_id == manufacturer && item.texture_id == texture && item.color == color && item.width == width_final)


        newClient[newClient.length - 1].color = temp_.color;
        newClient[newClient.length - 1].texture = temp_.texture
        newClient[newClient.length - 1].manufacturer = temp_.manufacturer
        newClient[newClient.length - 1].price = temp_.price
        newClient[newClient.length - 1].material = temp_.name
        newClient[newClient.length - 1].hex = temp_.hex
        newClient[newClient.length - 1].lines_length = lines_length
        newClient[newClient.length - 1].koordinats_poloten = koordinats_poloten
        newClient[newClient.length - 1].room_area = room_area
        newClient[newClient.length - 1].perimeter = perimeter
        newClient[newClient.length - 1].angles_count = angles_count
        newClient[newClient.length - 1].perimeter_shrink = perimeter_shrink
        newClient[newClient.length - 1].curvilinear_length = curvilinear_length
        newClient[newClient.length - 1].inner_cutout_length = inner_cutout_length
        newClient[newClient.length - 1].texture_id = temp_.texture_id
        newClient[newClient.length - 1].manufacturer_id = temp_.manufacturer_id
        newClient[newClient.length - 1].width_final = width_final
        newClient[newClient.length - 1].square_obrezkov = square_obrezkov
        newClient[newClient.length - 1].drawing_data = drawing_data
        newClient[newClient.length - 1].smeta = [];


        hideShow('.project-management', '.smeta_')
        getContent = getAjax({client: newClient[newClient.length - 1], success: 'currentDrawing'});
        userClient = getContent.client
        userClientAdress = getContent.clientAdress
        editUserClient = getContent.getClient
        getSelectList(editUserClient)


        // localStorage.setItem('editUserClient', JSON.stringify(editUserClient));
    ...
    function ok_enter_all() {
        if (manual_diag !== undefined) {
            ...
            manual_diag.data.unfixed_length = elem_newLength.value - 0;
        ...
        if (line_arc_height !== undefined && !line_arc_height.data.fixed) {
            ...
            button_reverse_arc = undefined;
            var dec_length = new Decimal(elem_newLength.value - 0), Dx, Dy,
    ...
    var ok_enter = function () {
        ...
        ready = false;
        var k, str_length = elem_newLength.value - 0;
    ...
    function clicks() {
        var audio = new Audio();
        audio.src = '/template/client/sounds/click.mp3';
	...
	square_tkan = new Decimal(Math.abs(chertezh.area)).dividedBy(ONE_M_SQ).toNumber();
	...
	while(c_i < chertezh.segments.length) {
    	...
    	for (var i = lines_tkan.length; i--;) {
        	for (var j = lines_tkan[i].segments.length; j--;) {
        		if (sketch.comparePoints(p, lines_tkan[i].segments[j].point, HALF)) {
	...
	for (var angle_rotate = 0; angle_rotate < 360; angle_rotate++) {
		for (var j = 0; j < widthsOfCanvases.length; j++) {
			...
			var points_m = sketch.findMinAndMaxCordinate(g_points);
			if (widthsOfCanvases[j].width < Math.abs(points_m.maxY - points_m.minY)) {
				continue;
			}
			add_polotno_tkan(widthsOfCanvases[j].width);
			sq_polo = new Decimal(polotna[0].down.length).times(polotna[0].left.length);
			sq_polo = sq_polo.dividedBy(ONE_M_SQ).toNumber();
	...
	g_points = getPathsPoints(lines_tkan);
	add_polotno_tkan(widthsOfCanvases[j_f].width);
    ...
    for (var i = 0; i < lines_tkan.length; i++) {
    	...
    	if (lines_tkan[i] !== lines_tkan[j] && sketch.commonPoint(lines_tkan[i], lines_tkan[j]) !== null) {
    		moveVertexNameContur(lines_tkan[i], lines_tkan[j], sketch.commonPoint(lines_tkan[i], lines_tkan[j]));
    	}
    }
	...
	width_final = widthsOfCanvases[j_f].width;
	sq_polotna = sq_polo;
	////console.log(width_final, "widthsOfCanvases");
...
function add_polotno_tkan(p_width) {
	...
	polotna = [];
	var points_m = sketch.findMinAndMaxCordinate(g_points);
	var line_left = Path.Line(new Point(points_m.minX, points_m.maxY),
		new Point(points_m.minX, new Decimal(points_m.maxY).minus(p_width).toNumber()));
	var line_right = Path.Line(new Point(points_m.maxX, points_m.maxY),
		new Point(points_m.maxX, new Decimal(points_m.maxY).minus(p_width).toNumber()));
...
function moveVertexNameContur(line1, line2, newPoint) {
    ...
    $('#btn_select_client').click(function () {
        ...
        if (selectedAdress = $('#get_select_client option:selected').attr('data_adress')) {
            getUserClientData = userClient.find(item => item.id === clientId)

            editUserClient = getAjax({adress_id: selectedAdress, obj: getUserClientData, success: 'getClient'})

            $('#client').val(editUserClient[0].name);
            $('#client_phone').val(editUserClient[0].phone);
            $('#client_adress').val(editUserClient[0].adress);
            $('#popup_client_select').hide()
            $('#popup_client').show()
            hideShow('.main', '.smeta_')
            getSelectList(editUserClient)
        } else if (userClientAdress.filter(item => item.client_id === clientId).length > 1) {
            selectClient('adress', clientId)
        } else {

            client = userClient.find(item => item.id === clientId)['name'];
            phone = userClient.find(item => item.id === clientId)['phone'];

            adress = userClientAdress.find(item => item.client_id === clientId).adress
            adress_id = userClientAdress.find(item => item.client_id === clientId).id

            $('#client').val(client);
            $('#client_phone').val(phone);
            $('#client_adress').val(adress);

            getUserClientData = userClient.find(item => item.id === clientId)

            editUserClient = getAjax({adress_id: adress_id, obj: getUserClientData, success: 'getClient'})
    ...
    $('#btn_select_client_to').click(function () {
        clientAdresId = $('#get_select_client_to').val();
        clientAdresName = $('#get_select_client_to option:selected').text();

        client_id = userClientAdress.find(item => item.id === clientAdresId).client_id

        getUserClientData = userClient.find(item => item.id === client_id)

        editUserClient = getAjax({adress_id: clientAdresId, obj: getUserClientData, success: 'getClient'})


        $('#client').val(getUserClientData.name);
        $('#client_phone').val(getUserClientData.phone);
        $('#client_adress').val(clientAdresName)
    ...
    //////////////////////////////////////////////
    $(document).on('click', '.oneCeiling .multiplied,.oneCeiling .multipliedwork', function () {
        if ($(this).hasClass('active') ) {

        }else if(currentTypeClient == 1){
            return false
        }else{
            clossedInput()
            $('.multiplied ,.multipliedwork').removeClass('active')
            $(this).addClass('active')

            edittype = $(this).attr('data_edittype')
            testValue = ($(this).text()-0).toFixed(2)

            $(this).empty().append('<input type="number"   id="inputActive" value="' + testValue + '">')
            $('#inputActive').select()
            calculate();
    ...
    $(document).on('change keyup paste','.multiplied input,.multipliedwork input',function(e) {
        res = $(this).val()
        editType = $(this).parent().attr('data_edittype')

            if(typeof editUserClient.find(it => it.id === ceilingId).editprice === 'string' && editUserClient.find(it => it.id === ceilingId).editprice != 'null'){

                editprice = JSON.parse(editUserClient.find(it => it.id === ceilingId).editprice)

            }
        // if(editType.includes("id-")){
        //     editType = editType.replace('id-', '')
        // }

        if (!$.isEmptyObject(editObject)) {
            if(editObject[ceilingId] === undefined){
                editObject[ceilingId] = []
                editObject[ceilingId][editType] =  res
            }else{
                if(editObject[ceilingId][editType] === undefined){
                    editObject[ceilingId][editType] = res
                }else{
                    editObject[ceilingId][editType] = res
                }
        ...
        editObject[ceilingId] = getAjax({
            ceilingId: ceilingId,
            editType: editType,
            res: res,
            success: 'editPrice'
        });
        editUserClient.find(it => it.id === ceilingId).editprice = editObject[ceilingId]

// //console.log(editObject)
// //console.log(editUserClient)
    })
    function clossedInput(){
        $('.oneCeiling .multiplied').each(function(){
            res = $(this).children('input').val()

            if (res !=undefined){
                $(this).empty().append((res-0).toFixed(2))
            }


        })
        $('.oneCeiling .multipliedwork').each(function(){
            res = $(this).children('input').val()
    ...
    $(document).on('change', '#selectRoom,#select_room_client,#select_smeta_client', function (e) {
        itemSelect = $(this).val()
        console.log(itemSelect)
        getSelectList(editUserClient, itemSelect)
    ...
    function getBigId(arr) {

        return arr.reduce((acc, curr) => acc.id > curr.id ? acc : curr).id;
    }

    function getKeyByValue(object, value) {

        return Object.keys(object).find(key => object[key] === object.find(item => item.id === value));
    }


    if (setClient != null) {
        $('#client_adress').val(userClientAdress[0].adress)
        hideShow('.project-management', '.smeta_')
        editUserClient = setClient
        $(this).css('text-decoration', 'underline').addClass('activeCeiling')
        $('#oneCeiling').css('text-decoration', 'none').removeClass('activeCeiling')
        $('.oneCeiling,.vcladka,.vcladka-2,.smeta_').hide()
        $('.smeta_').remove()
        $('.allCeiling').show()
        $('.project-management').css('margin-top','0')

        fillTable(editUserClient)
        currentType = 1

        calculate('allCeiling')
    }
    function fillTable(arr) {
        ...
        $.each(arr, function (ind, item) {
            drawing_data_rec = editUserClient[ind]['drawing_data']


            images = getImgCanvas(drawing_data_rec_(sketch, JSON.parse(drawing_data_rec)), item.id)

            // шапка
            numberOrder = item.id_user + '-' + item.client_id + '-' + item.adress_id
            $(' .number_order').empty().append(" <a href='/client/" + numberOrder + "'>" + numberOrder  + "</a>");

            $(' .clientSmeta').text(userClient.find(it => it.id === item.client_id).name)
            $(' .client_adressSmeta').text(userClientAdress.find(it => it.client_id === item.client_id && it.adress == $('#client_adress').val()).adress)
            $(' .client_phoneSmeta').text(userClient.find(it => it.id === item.client_id).phone)
            $(' .day').text(item.days);

            // ТАБЛИЦА

            str += "<thead class='thead rooms'> <tr><td class='tdroom' colspan='5' style='font-size: 16px;font-weight: bold;'>Помещение: " + rooms.find(it => it.id == item.room_id).name + "</td></tr> "
            str += "<tr><td  colspan='5' style='font-size: 16px;font-weight: bold;'><div class='w-90' style='text-align: center;margin: 0 auto'><img  style='max-width: 100%;' src=' "+ images +" '></div></td></tr>"


            str += appendSmetaMaterialy(item)

            str += appendSmetaMaterialy(item,1)

            str += "</tbody>"

        })
        str += "<tbody><tr> <td colspan='4' class='fw-bold fz-12 ta-right ' style='text-align: right;'>Итого всего:</td> <td class='fw-bold fz-12 ta-right itogovsego'></td> </tr></tbody>"
        $('.allCeiling .table').empty().append(str)
        editRedactorPriceAll()
        redactionTable()
        calculate()
    }
    $(document).click(function (e) {
if(onclickTrue == 0 ){
    if (!$(e.target).closest(".oneCeiling .multiplied,.oneCeiling .multipliedwork").length ) {
        clossedInput()
        calculate()
    }
}
    ...
    function redactionTable(){
        if($('.tbody:eq(1) tr').length == 3){
            $('.tbody:eq(1)').remove()
        }
    }

    function getSelectList(arr, item = false) {
        ceilingId = item;
        $('#selectRoom,#select_room_client,#select_smeta_client').empty()

        if (item === false) {
            ceilingId = getBigId(arr)
        }
        ...
        $.each(arr, function (ind, val) {
            ...
            if (ceilingId == val.id) {
                ...
                listGroop(types_of_work, currentTypeView,editUserClient[ind]) //формирование таблицы для составления сметы
                addInListGroopsSmeta(editUserClient[ind]) // открывает группы если в них что то есть
                specification(editUserClient[ind]) // заполняет спецификацию
                oneSmeta(editUserClient[ind], ind); //заполняет смету
                editRedactorPriceAll() // редактируем смету если были изменения

                getPlaceholder(editUserClient[ind])
                redactionTable()


                $('#selectRoom,#select_room_client,#select_smeta_client').append("<option selected value='" + val.id + "'>" + rooms.find(it => it.id == val.room_id).name + "</option>")
            } else {
                $('#selectRoom,#select_room_client,#select_smeta_client').append("<option value='" + val.id + "'>" + rooms.find(it => it.id == val.room_id).name + "</option>")
            }
        })

        getImgCanvas(drawing_data_rec_(sketch, JSON.parse(drawing_data_rec)))
        calculate()
    ...
    function getPlaceholder(item){
        // console.log(item)
        //
        // property = editUserClient.find(it => it.id === ceilingId)[$('#id-' + inputId).attr('data_property')];
        // $('onsmetaId-'+inputId).attr('placeholder',property)
    }

    function editRedactorPriceAll(ceiling = false){

        $.each(editUserClient, function (ind, item) {

            if(typeof item.editprice === "string" && item.editprice != 'null'){
                item.editprice = JSON.parse(item.editprice)
            }


            if (!$.isEmptyObject(item.editprice) && currentTypeClient == 0){
                $.each(item.editprice, function (i, items) {

                    $('.itemId-' + item.id +'  .multiplied[data_edittype=' + i +']').text((items-0).toFixed(2))
                })
            }
    ...
    function editRedactorPrice(arr = false){
        editObject = editUserClient.find(it => it.id === ceilingId).editprice
            if(typeof editObject === "string" && editObject != 'null'){
                editObject = JSON.parse(editObject)
            }
            if (editObject != 'null'){
                if (!$.isEmptyObject(editObject)) {
                    $.each($('.multiplied'), function (ind, val) {

                        attr = $(val).attr('data_edittype')

                        if(editObject[attr] !== undefined ){

                            $(val).text((editObject[attr]-0).toFixed(2))
                        }
                    })
                }
    ...
    $('#materialCeilingSmeta').on('click', function () {

        item = $(this).text()
        if (item == 'Работы') {
            currentTypeView = 0
            listGroop(types_of_work, 0)
            $(this).text('Материалы')
        }
        if (item == 'Материалы') {
            currentTypeView = 1
            listGroop(types_of_work, 1)
            $(this).text('Работы')
        }
        addInListGroopsSmeta(editUserClient.find(it => it.id === ceilingId))

    })

        $('#currentTypeClient').click(function () {
            currentTypeClient = 0

            if(currentType == 0)getSelectList(editUserClient, ceilingId)
            if(currentType == 1)fillTable(editUserClient)
        })
        $('#currentTypeMontaj').click(function () {
            currentTypeClient = 1
            if(currentType == 0)getSelectList(editUserClient, ceilingId)
            if(currentType == 1)fillTable(editUserClient)
    ...
    $('#oneCeiling').click(function () {
        $(this).css('text-decoration', 'underline').addClass('activeCeiling')

        $('#allCeiling').css('text-decoration', 'none').removeClass('activeCeiling')
        $('.allCeiling').hide()
        $('.oneCeiling').show()
        getSelectList(editUserClient, ceilingId)
        currentType = 0
        $('.allCeiling .table').empty()
    })

    $('#allCeiling').click(function () {
        $(this).css('text-decoration', 'underline').addClass('activeCeiling')
        $('#oneCeiling').css('text-decoration', 'none').removeClass('activeCeiling')
        $('.oneCeiling').hide()
        $('.allCeiling').show()
    ...
    function addInListGroopsSmeta(item) {

        if (typeof item.smeta === 'string' && item.smeta != 'null') {
            item.smeta = JSON.parse(item.smeta)
        }
        if (item.smeta != 'null') {

            if (Object.entries(item.smeta).length !== 0) {

                $.each(item.smeta, function (ind, val) {

                    if (val != undefined) {

                        if (!$('#id-' + val.id).parent('.sub-nav').hasClass('visible')) {
                            $('#id-' + val.id).closest('.sub-nav').addClass('visible').slideDown(300);
                            $('#id-' + val.id).val(val.quantiny)
                        }
                    }
                })
            }
    ...
    function calculate(clas = false) {
        itogo = 0
        itogo_work = 0

        if(!clas){
            clas = '.' + $('.activeCeiling').attr('id');
        }else{
            clas = '.' + clas
        }
        $(clas + ' .tbody').each(function (i, obj) {
            res = 0
            res_work = 0

            $(obj).children('.calculate ').each(function (ii, obj_) {

                multiplied = $(obj_).find('.multiplied ').text()
                multiplied_work = $(obj_).find('.multipliedwork ').text()

                multiplier = $(obj_).find('.multiplier ').text()
                multiplier_work = $(obj_).find('.multiplierwork ').text()
                $(obj_).find('.res ').text((multiplied * multiplier).toFixed(2))
                $(obj_).find('.reswork ').text((multiplied_work * multiplier_work).toFixed(2))
                res = multiplied * multiplier + res
                res_work = multiplied_work * multiplier_work + res_work


            })
            $(obj).find(' .endTable .itog ').text(res.toFixed(2))
            $(obj).find(' .endTablework .itogwork ').text(res_work.toFixed(2))

            itogo = (itogo + res)
            itogo_work = (itogo_work + res_work)



        });
        itogovsego = (itogo+itogo_work).toFixed(2)

        $('.itogovsego').text(itogovsego)
    }



    function setProperty(num,multiple){
...
function inputIdReturn(input,inputId){
    inputId = input.attr('id').replace('id-', '')
    price = input.attr('data_price')
    price_montaj = input.attr('data_price_montaj')
    multiple = input.attr('data_multiple')
    inputValue = setProperty(inputValue,multiple)
    console.log(inputValue)
    addSmeta(inputId, (inputValue - 0).toFixed(2), price - 0, price_montaj - 0)
    calculate()
    addInListGroopsSmeta(editUserClient.find(it => it.id === ceilingId))
}
    function getObject(obj){
    ...
    function appendSmetaMaterialy(item,kind = false){
        str = '';
        $str = '';
        if(kind == 0){
            typeWork  = 'Работы'
        ...
        if(kind === false) kind = 0;

        if(item.smeta = getObject(item.smeta)){

            $.each(item.smeta, function (ind, val) {


                if(typeof val != 'string' && val != '' && val != undefined){

                    $.each(types_of_work, function (work_key,work_val) {
                        if(work_val.find(it => it.id === val.id) !== undefined){
                            if (work_val.find(it => it.id === val.id).kinds == kind){
                                    result = work_val.find(it => it.id === val.id)


                                if(currentTypeClient == 0){
                                ...
                                if(currentTypeClient == 1){

                                    item_price = item.price_montage
                                    price = result.price_montaj
                                }
                                if(currentTypeClient == 2){

                                    price = result.price_diler
                                }
                                str += "<tr class='calculate'><td>" + result.name + "</td><td class='onsmetaId-" + result.id + " multiplier" + flag + "'>" + val.quantiny + "</td><td>" + unit[result.id_unit_of_measurement] + "</td><td class='multiplied" + flag + "' data_edittype='id-" + result.id + "'>" + price + "</td><td class='total res" + flag + "'></td></tr>"
        ...
        if (kind == 1 && str == '' ){
            ...
            if (currentTypeClient == 1 && kind == 1){
                $str += ""
            }else{

                    $str += "<tbody class='tbody itemId-"+item.id+"'><tr><td  colspan='5' style='font-size: 16px;font-weight: bold;'>" + typeWork + "</td></tr><tr><th scope='col'>Наименование</th><th scope='col'>Кол-во</th><th scope='col'>Ед.</th> <th scope='col'>Цена</th><th scope='col'>Сумма</th></tr>"
    ...
    if(kind == 0){
        $str += "<tr class='calculate'><td id='one_smeta_manufacturer' class='tdManufacture'>" + item.manufacturer + "</td><td id='one_smeta_room_area' class='multiplier" + flag + "'>" + Number(item.room_area).toFixed(2) + "</td> 	 			<td>M<sup>2</sup></td> 	 	 			<td id='one_smeta_price' class='multiplied" + flag + "' data_editType='room_area'>" + Number(item_price).toFixed(2) + "</td> 	 			<td class='one_smetatotal res" + flag + "'></td> 	 		</tr>"
        if(currentTypeClient == 0){
            $str += dopUslugi(item);
        }
    ...
    if(kind == 0 ){
        ...
        $str += "<tr class='endTable'> <td style='text-align: right;font-weight: bold' class='tdItog' colspan='4'>Итого</td> <td style='font-weight: bold' class='itog '></td> </tr>"
    }
    if(kind == 1 && str != '' && currentTypeClient == 0){
        $str += "<tr class='endTablework'> <td style='text-align: right;font-weight: bold' class='tdItog' colspan='4'>Итого</td> <td style='font-weight: bold' class='itogwork '></td> </tr>"
    }
    ...
    function oneSmeta(item, key = false) {
        ...
        // шапка
        numberOrder = item.id_user + '-' + item.client_id + '-' + item.adress_id
        $(' .number_order').empty().append(" <a href='/client/" + numberOrder + "'>" + numberOrder + key + "</a>");
        $(' .clientSmeta').text(userClient.find(it => it.id === item.client_id).name)
        $(' .client_adressSmeta').text(userClientAdress.find(it => it.client_id === item.client_id && it.adress == $('#client_adress').val()).adress)
        $(' .client_phoneSmeta').text(userClient.find(it => it.id === item.client_id).phone)
        $(' .day').text(item.days);
...
console.log(item)
        str = "<thead class='thead rooms'> <tr><td class='tdroom' colspan='5' style='font-size: 16px;font-weight: bold;'>Помещение: " + rooms.find(it => it.id == item.room_id).name + "</td></tr> </thead> "
        ...
        $('.oneCeiling .table').empty().append(str)
    }
    function addSmeta(inputId, inputValue, price, price_montaj) {


        if (editUserClient.find(it => it.id === ceilingId).smeta == 'null') {
            editUserClient.find(it => it.id === ceilingId).smeta = []
        }

        if (Number(inputValue == 0) && editUserClient.find(it => it.id === ceilingId).smeta[inputId] != undefined) {

            delete editUserClient.find(it => it.id === ceilingId).smeta[inputId]
            if ($('.tbody tr td').hasClass('onsmetaId-' + inputId)) {
                $('.onsmetaId-' + inputId).parent('tr').remove()
            }

            if($('.tbody:eq(1) tr').length == 3){
                $('.tbody:eq(1)').remove()
            }
        } else if(Number(inputValue == 0) && editUserClient.find(it => it.id === ceilingId).smeta[inputId] == undefined){
            ...
            addSmetaArr = {
                ...
                quantiny: inputValue,
                price: price,
                price_montaj: price_montaj,
                edit_price: false
            }

            editUserClient.find(it => it.id === ceilingId).smeta[inputId] = addSmetaArr
            oneSmeta(editUserClient.find(it => it.id === ceilingId),editUserClient.find(it => it.id === ceilingId).length)

            if (!$('.tbody tr td').hasClass('onsmetaId-' + inputId)) {
                text = $('#id-' + inputId).closest('.wid-6').text();
                price = $('#id-' + inputId).attr('data_price');
                units = $('#id-' + inputId).attr('data_unit');
                price_montaj = $('#id-' + inputId).attr('data_price_montaj');
                multiple = $('#id-' + inputId).attr('data_multiple');

                if(currentTypeView == 0)endTable = $('.endTable')
                if(currentTypeView == 1)endTable = $('.endTablework')
                endTable.before("<tr class='calculate'><td>" + text + "</td><td class='onsmetaId-" + inputId + " multiplier'>" + inputValue + "</td><td>" + units + "</td><td class='multiplied' data_editType='id-" + inputId + "'>" + (price - 0).toFixed(2) + "</td><td class='total res'></td></tr>")
            }
        }

        $('.onsmetaId-' + inputId).text(inputValue)

        res = getAjax({
            ceilingId: ceilingId,
            data: editUserClient.find(it => it.id === ceilingId).smeta,
            success: 'addSmeta'
        });
    ...
    function dopUslugi(item, str = false) {
        if (!str) str = ''

        if (item.square_obrezkov > 2) {

            str += "<tr class='calculate'><td>Обрезки </td><td class='square_obrezkov multiplier' > " + Number(item.square_obrezkov).toFixed(2) + " </td><td>м<sup>2</sup></td><td class='multiplied' data_editType='square_obrezkov'>" + Number(types_of_work['Доп. работы производства'].find(it => it.nameas === "square_obrezkov").price).toFixed(2) + "</td><td class='total res'></td></tr>"
        }
        if (item.angles_count > 4) {
            ...
            if (Number(item.angles_count) - 4 != 0) {
                str += "<tr class='calculate'><td>Дополнительные углы (больше 4х) </td><td class='angles_count multiplier' > " + (Number(item.angles_count) - 4).toFixed(2) + " </td><td>м<sup>2</sup></td><td class='multiplied' data_editType='angles_count'>" + Number(types_of_work['Доп. работы производства'].find(it => it.nameas === "angles_count").price).toFixed(2) + "</td><td class='total res'></td></tr>"

            }
        }
        if (item.curvilinear_length != 0) {

            str += "<tr class='calculate'><td>Криволинейный участок</td><td class='curvilinear_length multiplier' > " + (Number(item.curvilinear_length)).toFixed(2) + " </td><td>м<sup>2</sup></td><td class='multiplied' data_editType='curvilinear_length'>" + Number(types_of_work['Доп. работы производства'].find(it => it.nameas === "curvilinear_length").price).toFixed(2) + "</td><td class='total res'></td></tr>"

        }
        if (item.inner_cutout_length != 0) {
            str += "<tr class='calculate'><td>Внутренний вырез</td><td class='inner_cutout_length multiplier' > " + (Number(item.inner_cutout_length)).toFixed(2) + " </td><td>м<sup>2</sup></td><td class='multiplied' data_editType='inner_cutout_length'>" + Number(types_of_work['Доп. работы производства'].find(it => it.nameas === "inner_cutout_length").price).toFixed(2) + "</td><td class='total res'></td></tr>"
        }
    ...
    function specification(item) {
        if (typeof item.lines_length !== 'object') {
            item.lines_length = JSON.parse(item.lines_length)
        }
        ...
        $('#manufacturer_id').text(item.manufacturer);
    ...
    function listGroop(listgroops, items = false,editUserClient) {

        str = ''


        $.each(listgroops, function (ind, val) {
            if (val.find(item => item.kinds == items)) {
                str += "<div  class='parent'><div class='par'>" + ind + "</div><div class='sub-nav'>"

                $.each(val, function (indV, valV) {
                    if (items == valV.kinds) {

                        if (valV['property'] !== null){

                            placeholder = "placeholder='"+ editUserClient[valV['property']] +"'"
                        }else{
                            placeholder = ''
                        }
                        str += "<div class='wid-6 fz-20'><div class='name w-75' title='" + valV.description + "'>" + valV.name + "</div><div class='val w-10'><input class='inputid' " + placeholder + " id='id-" + valV.id + "' data_unit='" + unit[valV.id_unit_of_measurement] + "' data_price='" + valV.price + "' data_property='" + valV.property + "' data_kinds='" + valV.kinds + "' data_price_diler='" + valV.price_diler + "' data_price_montaj='" + valV.price_montaj + "' data_multiple='" + valV.multiple + "' style='width: 85px' type='number'></div></div>"
                    }
                })
                str += "</div></div>"
            }
    ...
    $('.block a').click(function () {
        ...
        $('.mains,.main,.document,.project-management,.listing,.user').hide()
        $('.fixed .row div img').css({"width": "32px", "border": "1px solid black"})
        $(this).children("img").css({"width": "36px", "border": "2px solid blue"})
        $(href).show()
        return false;
    })
    ...
    $('#btn_client_select').click(function () {
        ...
        $('#btn_client').removeClass('activity')
        $('#popup_client').hide()
        $('#popup_client_select').show().addClass('activity')

        selectClient('name');

    })

    $('#btn_select_client_adress').click(function () {

        if ($('#popup_client_select').hasClass('activity')) {
            $('#popup_client_select').removeClass('activity')
            $('.selectClient').text('Выберите адрес:')
            $('#btn_select_client_adress').text('По клиенту')
            selectClient('adress');
        } else {

            $('#popup_client_select').addClass('activity')
            $('.selectClient').text('Выберите клиета:')
            $('#btn_select_client_adress').text('По адресу')
    ...
    // if (drawing_data_rec !== undefined) {
    //    console.log(drawing_data_rec)
    //     drawing_data_rec_(sketch, drawing_data_rec)
    // }if (drawing_data_rec !== undefined) {
    //    console.log(drawing_data_rec)
    //     drawing_data_rec_(sketch, drawing_data_rec)
    // }
